...
struct RefLineP1
{
  ...
  static std::array<scalarFun_T,numPts> const phi;
  static std::array<vectorFun_T,numPts> const dphi;
  static localMat_T const massMat;
  static localMat_T const gradMat;
  static double constexpr volume = 2.L;
};

std::array<scalarFun_T,RefLineP1::numPts> const RefLineP1::phi =
{
  [] (Vec3 const & p) { return 0.5*(1-p(0)); },
  [] (Vec3 const & p) { return 0.5*(1+p(0)); }
};

std::array<vectorFun_T,RefLineP1::numPts> const RefLineP1::dphi =
{
