    ...
    long double const tolerance)
{
  ...
  using FESpaceP0Scalar_T = FESpace<Mesh_T, typename LagrangeFE<Elem_T, 0>::RefFE_T, QR>;
  ...
  auto const inputFun3d = [&inputFun] (Vec3 const & p) { return promote<3>(inputFun(p)); };
  ...
  builder.buildCoupling(AssemblyVectorGrad(1.0, feSpaceRT0, feSpaceLambda), bcs, std::tuple{});
  builder.buildCoupling(AssemblyVectorDiv(1.0, feSpaceLambda, feSpaceRT0), std::tuple{}, bcs);
  builder.buildRhs(std::tuple{AssemblyProjection(1.0, uP1.data, feSpaceP1, feSpaceRT0)}, bcs);
  builder.closeMatrix();
  std::cout << "A:\n" << builder.A << std::endl;
  std::cout << "b:\n" << builder.b << std::endl;
  LUSolver solver;
